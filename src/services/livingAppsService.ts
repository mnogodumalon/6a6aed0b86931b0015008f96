...
import type { Unternehmen, Termine, Dokumente, Notizen, CreateUnternehmen, CreateTermine, CreateDokumente, CreateNotizen } from '@/types/app';
...
/** What create* and update* resolve to. Same `record_id` the read helpers
...
export class LivingAppsService {
  ...
  static async getUnternehmenEntry(id: string): Promise<Unternehmen | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Unternehmen;
    return enrichLookupFields([record], 'unternehmen')[0];
  }
  static async createUnternehmenEntry(fields: CreateUnternehmen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.UNTERNEHMEN}/records`, { fields: cleanFieldsForApi(fields as any, 'unternehmen') });
    return { ...data, record_id: data.id };
  }
  static async updateUnternehmenEntry(id: string, fields: Partial<CreateUnternehmen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.UNTERNEHMEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'unternehmen') });
    return { ...data, record_id: data.id };
  }
  ...
  static async getTermine(): Promise<Termine[]> {
    ...
    return enrichLookupFields(records, 'termine');
  }
  static async getTermineEntry(id: string): Promise<Termine | undefined> {
    ...
    return enrichLookupFields([record], 'termine')[0];
  }
  ...
  static async getDokumenteEntry(id: string): Promise<Dokumente | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Dokumente;
    return enrichLookupFields([record], 'dokumente')[0];
  }
  static async createDokumenteEntry(fields: CreateDokumente): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.DOKUMENTE}/records`, { fields: cleanFieldsForApi(fields as any, 'dokumente') });
    return { ...data, record_id: data.id };
  }
  static async updateDokumenteEntry(id: string, fields: Partial<CreateDokumente>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.DOKUMENTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'dokumente') });
    return { ...data, record_id: data.id };
  }
