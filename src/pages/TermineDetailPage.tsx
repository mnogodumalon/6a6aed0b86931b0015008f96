...
export default function TermineDetailPage() {
  ...
  const [record, setRecord] = useState<Termine | null>(null);
  ...
  async function handleUpdate(fields: Termine['fields']) {
    if (!record) return;
    await LivingAppsService.updateTermineEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteTermineEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/termine');
  }
