...
export default function TerminePage() {
  ...
  const [records, setRecords] = useState<Termine[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Termine | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Termine | null>(null);
  ...
  async function handleCreate(fields: Termine['fields']) {
    await LivingAppsService.createTermineEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Termine['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateTermineEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteTermineEntry(deleteTarget.record_id);
