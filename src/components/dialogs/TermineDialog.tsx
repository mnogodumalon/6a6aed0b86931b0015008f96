 ...
 * unternehmenList (full hook array — resolves the Unternehmen applookup),
...
import type { Termine, Unternehmen, LookupValue } from '@/types/app';
...
export type TermineDialogDefaults = Omit<Termine['fields'], 'terminart' | 'wiederholung' | 'terminstatus'> & {
    terminart?: LookupValue | string;
    wiederholung?: LookupValue | string;
    terminstatus?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  terminart: LOOKUP_OPTIONS['termine']?.['terminart'] ?? [],
  wiederholung: LOOKUP_OPTIONS['termine']?.['wiederholung'] ?? [],
  terminstatus: LOOKUP_OPTIONS['termine']?.['terminstatus'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  unternehmen: APP_IDS.UNTERNEHMEN,
};
...
export function TermineDialog({ open, onClose, onSubmit, defaultValues, recordId, unternehmenList, enablePhotoScan = true, enablePhotoLocation = true }: TermineDialogProps) {
  ...
  const REQUIRED_FIELDS = ['unternehmen', 'terminbezeichnung', 'terminart', 'datum_uhrzeit'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "unternehmen": string | null, // Display name from Unternehmen (see <available-records>)\n  "terminbezeichnung": string | null, // Terminbezeichnung\n  "terminart": LookupValue | null, // Terminart (select one key: "gremiensitzung" | "gesellschafterversammlung" | "beiratssitzung" | "strategiemeeting" | "jahresabschluss" | "terminart_sonstiges") mapping: gremiensitzung=Gremiensitzung, gesellschafterversammlung=Gesellschafterversammlung, beiratssitzung=Beiratssitzung, strategiemeeting=Strategiemeeting, jahresabschluss=Jahresabschlussbesprechung, terminart_sonstiges=Sonstiges\n  "datum_uhrzeit": string | null, // YYYY-MM-DDTHH:MM\n  "ort": string | null, // Ort / Videolink\n  "wiederholung": LookupValue | null, // Wiederholung (select one key: "einmalig" | "woechentlich" | "monatlich" | "quartalsweise" | "jaehrlich") mapping: einmalig=Einmalig, woechentlich=Wöchentlich, monatlich=Monatlich, quartalsweise=Quartalsweise, jaehrlich=Jährlich\n  "erinnerung_tage": number | null, // Erinnerung (Tage vorher)\n  "google_kalender": boolean | null, // Mit Google-Kalender synchronisieren\n  "terminstatus": LookupValue | null, // Status (select one key: "geplant" | "stattgefunden" | "abgesagt") mapping: geplant=Geplant, stattgefunden=Stattgefunden, abgesagt=Abgesagt\n  "notizen_termin": string | null, // Notizen zum Termin\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["unternehmen"]);
        ...
        if (unternehmenName) {
          const unternehmenMatch = unternehmenList.find(r => matchName(unternehmenName!, [String(r.fields.name ?? '')]));
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'unternehmen': (
      <div key="unternehmen" className="space-y-1.5">
        ...
        <Combobox
          id="unternehmen"
          placeholder="Welches Unternehmen betroffen?"
    ...
    'terminbezeichnung': (
      <div key="terminbezeichnung" className="space-y-1.5">
        ...
        <Input
          ...
          placeholder="z. B. Board-Meeting Q3"
    ...
    'terminart': (
      <div key="terminart" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="terminart" className="max-sm:h-11"><SelectValue placeholder="Wähle die Terminart" /></SelectTrigger>
          <SelectContent>
    ...
    'ort': (
      <div key="ort" className="space-y-1.5">
        ...
        <Input
          id="ort"
          placeholder="z. B. Berlin, Zoom-Link oder Adresse"
          value={fields.ort ?? ''}
      ...
      <div key="wiederholung" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.wiederholung) === 'einmalig'}
            onClick={() => setFields(f => ({ ...f, wiederholung: (lookupKey(f.wiederholung) === 'einmalig' ? undefined : 'einmalig') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.wiederholung) === 'einmalig'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.wiederholung) === 'woechentlich'}
            onClick={() => setFields(f => ({ ...f, wiederholung: (lookupKey(f.wiederholung) === 'woechentlich' ? undefined : 'woechentlich') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.wiederholung) === 'woechentlich'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.wiederholung) === 'monatlich'}
            onClick={() => setFields(f => ({ ...f, wiederholung: (lookupKey(f.wiederholung) === 'monatlich' ? undefined : 'monatlich') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.wiederholung) === 'monatlich'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.wiederholung) === 'quartalsweise'}
            onClick={() => setFields(f => ({ ...f, wiederholung: (lookupKey(f.wiederholung) === 'quartalsweise' ? undefined : 'quartalsweise') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.wiederholung) === 'quartalsweise'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.wiederholung) === 'jaehrlich'}
            onClick={() => setFields(f => ({ ...f, wiederholung: (lookupKey(f.wiederholung) === 'jaehrlich' ? undefined : 'jaehrlich') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.wiederholung) === 'jaehrlich'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
      ...
      <div key="terminstatus" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.terminstatus) === 'geplant'}
            onClick={() => setFields(f => ({ ...f, terminstatus: (lookupKey(f.terminstatus) === 'geplant' ? undefined : 'geplant') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.terminstatus) === 'geplant'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.terminstatus) === 'stattgefunden'}
            onClick={() => setFields(f => ({ ...f, terminstatus: (lookupKey(f.terminstatus) === 'stattgefunden' ? undefined : 'stattgefunden') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.terminstatus) === 'stattgefunden'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.terminstatus) === 'abgesagt'}
            onClick={() => setFields(f => ({ ...f, terminstatus: (lookupKey(f.terminstatus) === 'abgesagt' ? undefined : 'abgesagt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.terminstatus) === 'abgesagt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'notizen_termin': (
      <div key="notizen_termin" className="space-y-1.5">
        ...
        <Textarea
          id="notizen_termin"
          placeholder="Agenda, Teilnehmer, Gesprächsergebnisse..."
  ...
  const FIELD_LABELS: Record<string, string> = {"unternehmen": "Unternehmen", "terminbezeichnung": "Terminbezeichnung", "terminart": "Terminart", "datum_uhrzeit": "Datum & Uhrzeit", "ort": "Ort / Videolink", "wiederholung": "Wiederholung", "erinnerung_tage": "Erinnerung (Tage vorher)", "google_kalender": "Mit Google-Kalender synchronisieren", "terminstatus": "Status", "notizen_termin": "Notizen zum Termin"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"unternehmen": {"name": "Unternehmensname", "rechtsform": "Rechtsform", "branche": "Branche", "status": "Status der Beteiligung", "beteiligungsquote": "Beteiligungsquote (%)", "investiertes_kapital": "Investiertes Kapital (EUR)", "aktueller_wert": "Aktueller Unternehmenswert (EUR)", "investitionsdatum": "Investitionsdatum", "stadt": "Stadt", "land": "Land", "website": "Website", "ansprechpartner_vorname": "Vorname Ansprechpartner", "ansprechpartner_nachname": "Nachname Ansprechpartner", "ansprechpartner_email": "E-Mail Ansprechpartner", "ansprechpartner_telefon": "Telefon Ansprechpartner", "cockpit_zusammenfassung": "Cockpit-Zusammenfassung", "allgemeine_notizen": "Allgemeine Notizen"}};
    ...
    {createUnternehmenOpen && (
      <UnternehmenDialog
        ...
        defaultValues={createUnternehmenInitial
          ? ({ name: createUnternehmenInitial } as any)
          : undefined}
      />
    )}
    </>
