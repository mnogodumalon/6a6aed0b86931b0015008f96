...
import type { Notizen, Unternehmen, LookupValue } from '@/types/app';
...
export type NotizenDialogDefaults = Omit<Notizen['fields'], 'kategorie' | 'prioritaet'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kategorie: LOOKUP_OPTIONS['notizen']?.['kategorie'] ?? [],
...
export function NotizenDialog({ open, onClose, onSubmit, defaultValues, recordId, unternehmenList, enablePhotoScan = true, enablePhotoLocation = true }: NotizenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['unternehmen', 'notiz_titel', 'notiz_inhalt', 'notiz_datum'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "unternehmen": string | null, // Display name from Unternehmen (see <available-records>)\n  "notiz_titel": string | null, // Titel der Notiz\n  "notiz_inhalt": string | null, // Notizinhalt\n  "notiz_datum": string | null, // YYYY-MM-DD\n  "kategorie": LookupValue | null, // Kategorie (select one key: "allgemein" | "meeting" | "finanzen" | "strategie" | "risiken" | "kategorie_sonstiges") mapping: allgemein=Allgemein, meeting=Meeting, finanzen=Finanzen, strategie=Strategie, risiken=Risiken, kategorie_sonstiges=Sonstiges\n  "prioritaet": LookupValue | null, // Priorität (select one key: "hoch" | "mittel" | "niedrig") mapping: hoch=Hoch, mittel=Mittel, niedrig=Niedrig\n  "schlagwoerter": string | null, // Schlagwörter / Tags\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'unternehmen': (
      <div key="unternehmen" className="space-y-1.5">
        <Label htmlFor="unternehmen">{fieldLabel('notizen', 'unternehmen')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="unternehmen"
          placeholder=""
    ...
    'notiz_titel': (
      <div key="notiz_titel" className="space-y-1.5">
        <Label htmlFor="notiz_titel">{fieldLabel('notizen', 'notiz_titel')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="notiz_titel"
          placeholder=""
          ...
          required
        />
        {showErrors && !fields.notiz_titel && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'notiz_inhalt': (
      <div key="notiz_inhalt" className="space-y-1.5">
        <Label htmlFor="notiz_inhalt">{fieldLabel('notizen', 'notiz_inhalt')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Textarea
          id="notiz_inhalt"
          placeholder=""
          ...
          rows={3}
        />
        {showErrors && !fields.notiz_inhalt && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'notiz_datum': (
      <div key="notiz_datum" className="space-y-1.5">
        <Label htmlFor="notiz_datum">{fieldLabel('notizen', 'notiz_datum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="notiz_datum"
          placeholder=""
          mode="date"
    ...
    'kategorie': (
      <div key="kategorie" className="space-y-1.5">
        <Label htmlFor="kategorie">{fieldLabel('notizen', 'kategorie')}</Label>
        <Select
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="allgemein">{lookupLabel('notizen', 'kategorie', 'allgemein') ?? 'Allgemein'}</SelectItem>
            <SelectItem value="meeting">{lookupLabel('notizen', 'kategorie', 'meeting') ?? 'Meeting'}</SelectItem>
            <SelectItem value="finanzen">{lookupLabel('notizen', 'kategorie', 'finanzen') ?? 'Finanzen'}</SelectItem>
            <SelectItem value="strategie">{lookupLabel('notizen', 'kategorie', 'strategie') ?? 'Strategie'}</SelectItem>
            <SelectItem value="risiken">{lookupLabel('notizen', 'kategorie', 'risiken') ?? 'Risiken'}</SelectItem>
            <SelectItem value="kategorie_sonstiges">{lookupLabel('notizen', 'kategorie', 'kategorie_sonstiges') ?? 'Sonstiges'}</SelectItem>
          </SelectContent>
        </Select>
      </div>
    ),
    ...
    'schlagwoerter': (
      <div key="schlagwoerter" className="space-y-1.5">
        <Label htmlFor="schlagwoerter">{fieldLabel('notizen', 'schlagwoerter')}</Label>
        <Input
          id="schlagwoerter"
          placeholder=""
  ...
  const FIELD_LABELS: Record<string, string> = {"unternehmen": "Unternehmen", "notiz_titel": "Titel der Notiz", "notiz_inhalt": "Notizinhalt", "notiz_datum": "Datum der Notiz", "kategorie": "Kategorie", "prioritaet": "Priorität", "schlagwoerter": "Schlagwörter / Tags"};
