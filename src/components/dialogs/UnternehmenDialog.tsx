/**
 * UnternehmenDialog — pre-generated create/edit dialog for Unternehmen.
 *
 ...
 * UnternehmenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Unternehmen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<UnternehmenDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Unternehmen, LookupValue } from '@/types/app';
...
/** Widened prefill type for UnternehmenDialog.defaultValues — see file header. */
export type UnternehmenDialogDefaults = Omit<Unternehmen['fields'], 'rechtsform' | 'branche' | 'status'> & {
    rechtsform?: LookupValue | string;
    branche?: LookupValue | string;
...
interface UnternehmenDialogProps {
  ...
  defaultValues?: UnternehmenDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  rechtsform: LOOKUP_OPTIONS['unternehmen']?.['rechtsform'] ?? [],
  branche: LOOKUP_OPTIONS['unternehmen']?.['branche'] ?? [],
  status: LOOKUP_OPTIONS['unternehmen']?.['status'] ?? [],
};
...
export function UnternehmenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: UnternehmenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['name', 'status'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Unternehmen['fields']>);
      setPreview(null);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "name": string | null, // Unternehmensname\n  "rechtsform": LookupValue | null, // Rechtsform (select one key: "gmbh" | "ag" | "gmbh_co_kg" | "ug" | "kg" | "ohg" | "einzelunternehmen" | "sonstige") mapping: gmbh=GmbH, ag=AG, gmbh_co_kg=GmbH & Co. KG, ug=UG (haftungsbeschränkt), kg=KG, ohg=OHG, einzelunternehmen=Einzelunternehmen, sonstige=Sonstige\n  "branche": LookupValue | null, // Branche (select one key: "energie" | "medien" | "beratung" | "branche_sonstige" | "technologie" | "gesundheit" | "finanzen" | "immobilien" | "handel" | "produktion") mapping: energie=Energie & Umwelt, medien=Medien & Kommunikation, beratung=Beratung & Dienstleistung, branche_sonstige=Sonstige, technologie=Technologie & Software, gesundheit=Gesundheit & Medizin, finanzen=Finanzen & Versicherung, immobilien=Immobilien, handel=Handel & E-Commerce, produktion=Produktion & Industrie\n  "status": LookupValue | null, // Status der Beteiligung (select one key: "aktiv" | "inaktiv" | "exit") mapping: aktiv=Aktiv, inaktiv=Inaktiv, exit=Exit\n  "beteiligungsquote": number | null, // Beteiligungsquote (%)\n  "investiertes_kapital": number | null, // Investiertes Kapital (EUR)\n  "aktueller_wert": number | null, // Aktueller Unternehmenswert (EUR)\n  "investitionsdatum": string | null, // YYYY-MM-DD\n  "stadt": string | null, // Stadt\n  "land": string | null, // Land\n  "website": string | null, // Website\n  "ansprechpartner_vorname": string | null, // Vorname Ansprechpartner\n  "ansprechpartner_nachname": string | null, // Nachname Ansprechpartner\n  "ansprechpartner_email": string | null, // E-Mail Ansprechpartner\n  "ansprechpartner_telefon": string | null, // Telefon Ansprechpartner\n  "cockpit_zusammenfassung": string | null, // Cockpit-Zusammenfassung\n  "allgemeine_notizen": string | null, // Allgemeine Notizen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'rechtsform': (
      <div key="rechtsform" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="rechtsform" className="max-sm:h-11"><SelectValue placeholder="Rechtsform wählen" /></SelectTrigger>
          <SelectContent>
    ...
    'branche': (
      <div key="branche" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="branche" className="max-sm:h-11"><SelectValue placeholder="Branche des Unternehmens" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
    ...
    'aktueller_wert': (
      <div key="aktueller_wert" className="space-y-1.5">
        ...
        <Input
          ...
          placeholder="z. B. 1200000 (in EUR)"
    ...
    'cockpit_zusammenfassung': (
      <div key="cockpit_zusammenfassung" className="space-y-1.5">
        ...
        <Textarea
          ...
          placeholder="Kurzübersicht für die Cockpit-Ansicht"
    ...
    'allgemeine_notizen': (
      <div key="allgemeine_notizen" className="space-y-1.5">
        ...
        <Textarea
          ...
          placeholder="Besonderheiten, Kontext, Hintergrund…"
